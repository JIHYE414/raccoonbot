...
class SyncSimRaccoonDataset:
    ...
    def make_push_plan(self, box_x, box_y, push_dist=0.05):
        z_push = 0.02
        z_above = 0.08
        target_x = box_x + push_dist
        return [
            [box_x, box_y, z_above, 0],
            [box_x, box_y, z_push, 0],
            [target_x, box_y, z_push, 0],
            [target_x, box_y, z_above, 0],
        ]

    def make_pick_and_place_plan(self, box_x, box_y, place_x, place_y):
        z_above = 0.10
        z_grasp = 0.02
        return [
            [box_x, box_y, z_above, 0],
            ...
            [place_x, place_y, z_above, 1],
            [place_x, place_y, z_grasp, 1],
            [place_x, place_y, z_grasp, 0],
            [place_x, place_y, z_above, 0],
        ]

    def make_stack_plan(self, box_x, box_y, stack_x, stack_y, stack_z=0.04):
        z_above = 0.10
        z_grasp = 0.02
        return [
            [box_x, box_y, z_above, 0],
            ...
            [box_x, box_y, z_grasp, 1],
            [box_x, box_y, z_above, 1],
            [stack_x, stack_y, z_above, 1],
            [stack_x, stack_y, stack_z, 1],
            [stack_x, stack_y, stack_z, 0],
            [stack_x, stack_y, z_above, 0],
        ]

    def make_swap_plan(self, box1_x, box1_y, box2_x, box2_y):
        z_above = 0.10
        z_grasp = 0.02
        tmp_x, tmp_y = 0.0, 0.12
        return [
            [box1_x, box1_y, z_above, 0],
            [box1_x, box1_y, z_grasp, 0],
            [box1_x, box1_y, z_grasp, 1],
            [box1_x, box1_y, z_above, 1],
            [tmp_x, tmp_y, z_above, 1],
            [tmp_x, tmp_y, z_grasp, 1],
            [tmp_x, tmp_y, z_grasp, 0],
            [box2_x, box2_y, z_above, 0],
            [box2_x, box2_y, z_grasp, 0],
            [box2_x, box2_y, z_grasp, 1],
            [box2_x, box2_y, z_above, 1],
            [box1_x, box1_y, z_above, 1],
            [box1_x, box1_y, z_grasp, 1],
            [box1_x, box1_y, z_grasp, 0],
            [tmp_x, tmp_y, z_above, 0],
            [tmp_x, tmp_y, z_grasp, 0],
            [tmp_x, tmp_y, z_grasp, 1],
            [tmp_x, tmp_y, z_above, 1],
            [box2_x, box2_y, z_above, 1],
...
def collect_dataset(
    ...
    instruction_template=None,  # None이면 다양한 템플릿 랜덤 사용
    keep_failed=False,
    use_viewer=False,
    ...
    instruction_templates = {
        ...
        "lift": [
            ...
            "hold up the {color} cylinder",
        ],
        "push": [
            "push the {color} cylinder",
            "slide the {color} cylinder forward",
            "move the {color} cylinder",
        ],
        "pick_and_place": [
            "pick up the {color} cylinder and place it",
            "move the {color} cylinder to the target",
            "put the {color} cylinder down",
        ],
        "stack": [
            "stack the {color} cylinder",
            "place the {color} cylinder on top",
            "put the {color} cylinder on the stack",
        ],
        "swap": [
            "swap the {color} cylinder",
            "exchange the {color} cylinder position",
            "switch the {color} cylinder",
        ],
    }
    ...
    try:
        while sum(success_counts.values()) < num_episodes and attempt_count < max_attempts:
            ...
            # 랜덤 태스크 타입 선택
            task_types = ["grasp", "lift", "push", "pick_and_place", "stack", "swap"]
            selected_task = str(rng.choice(task_types))
            if instruction_template is None:
                tmpl = str(rng.choice(instruction_templates[selected_task]))
            else:
                tmpl = instruction_template
                selected_task = "grasp"
            instruction = tmpl.format(color=target_color)
            ...
            try:
                ...
                print(
                    ...
                    f"task_type='{selected_task}' | color='{target_color}' | "
                ...
                print(
                    f"[Attempt {attempt_count:04d}] task_type='{selected_task}' | "

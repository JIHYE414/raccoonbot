...
def detect_color_from_webcam(cap, color):
    ret, frame = cap.read()
    if not ret:
        return None
    hsv = cv2.cvtColor(frame, cv2.COLOR_BGR2HSV)
    lo, hi = COLOR_RANGES_HSV[color]
...
def rollout(
    ...
    if webcam.isOpened():
        print("[YOLO] 웹캠 연결됨 - 색깔 자동 감지 활성화")
    else:
        webcam = None
        print("[YOLO] 웹캠 없음 - target_color 수동 지정")
    ...
    if webcam is not None:
        print("[YOLO] 웹캠으로 색깔 감지 중...")
        detected = None
        for _ in range(30):  # 3초 동안 시도
            for color in ["red", "blue", "green", "yellow"]:
                if detect_color_from_webcam(webcam, color):
                    detected = color
                    break
            if detected:
                break
            time.sleep(0.1)
        if detected:
            print(f"[YOLO] 감지된 색깔: {detected} → target_color 변경")
            target_color = detected
            instruction = instruction_template.format(color=target_color)
        else:
            print(f"[YOLO] 감지 실패 → 기존 target_color 유지: {target_color}")
